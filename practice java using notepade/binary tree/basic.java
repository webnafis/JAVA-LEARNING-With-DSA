...
class Basic {
    ...
    static class Tree {
        ...
        public static void levelorder(Node root) {
            ...
            // Queue<Node> q = new ArrayDeque<>();// cant use cause i shouldnt use here or
            // add here null
    ...
    // calculating height by nodes, if want to calculate by edges then minus 1 from
    // the ultimate result of height by nodes
    ...
    public static Info diameter(Node root) {
        ...
        return new Info(Math.max(Math.max(leftInfo.diam, rightInfo.diam), (leftInfo.height + rightInfo.height + 1)),
    ...
    public static Node lowestCommonAncestorApproach2(Node root, int n1, int n2) {// O(n) but space recursion stack which
                                                                                 // is also O(n)
    ...
    public static int minDistanceBetweenLowestCommonAncestorUtil(Node root, int n) {// binary tree
        if (root == null) {
            return -1;
        }
        if (root.data == n) {
        ...
        int left = minDistanceBetweenLowestCommonAncestorUtil(root.left, n);
        int right = minDistanceBetweenLowestCommonAncestorUtil(root.right, n);
        if (left == -1 && right == -1) {
            return -1;
        } else if (left == -1) {
            return right + 1;
    ...
    // would be easy if used static variable
    public static int minDistanceBetween2Nodes(Node root, int n1, int n2) {
        Node lca = lowestCommonAncestorApproach2(root, n1, n2);
        return minDistanceBetweenLowestCommonAncestorUtil(lca, n1)
                + minDistanceBetweenLowestCommonAncestorUtil(lca, n2);
    }
